...
## Spring Session
### JDBC Session
> 애플리케이션을 클러스터링(이중화) 하여 사용하는 경우 세션을 인메모리로 관리할 수 없다. 그런 경우 세션으로 Redis 를 사용하거나 RDBMS 를 사용하여 처리한다.  
> 여기서는 JDBC Session 을 통한 RDBMS 사용을 정리하였다.  
> 
> application.yml
> ```yaml
> spring:
...
>     store-type: jdbc
>     jdbc:
>         table-name: CUSTOM_NAMED_SESSION  # 기본 테이블 명은 SPRING_SESSION 이지만 사용자화할 수 있다.
> ```
> 
> RDBMS 에서 Session 으로 사용할 테이블 쿼리
> ```sql
> CREATE TABLE CUSTOM_NAMED_SESSION (
...
> 
> CREATE UNIQUE INDEX CUSTOM_NAMED_SESSION_IX1 ON CUSTOM_NAMED_SESSION (SESSION_ID);
> CREATE INDEX CUSTOM_NAMED_SESSION_IX2 ON CUSTOM_NAMED_SESSION (EXPIRY_TIME);
> CREATE INDEX CUSTOM_NAMED_SESSION_IX3 ON CUSTOM_NAMED_SESSION (PRINCIPAL_NAME);
> 
> CREATE TABLE CUSTOM_NAMED_SESSION_ATTRIBUTES (
>     SESSION_PRIMARY_ID CHAR(36) NOT NULL,
>     ATTRIBUTE_NAME VARCHAR(200) NOT NULL,
>     ATTRIBUTE_BYTES BLOB NOT NULL,
>     CONSTRAINT CUSTOM_NAMED_SESSION_ATTRIBUTES_PK PRIMARY KEY (SESSION_PRIMARY_ID, ATTRIBUTE_NAME),
>     CONSTRAINT CUSTOM_NAMED_SESSION_ATTRIBUTES_FK FOREIGN KEY (SESSION_PRIMARY_ID) REFERENCES CUSTOM_NAMED_SESSION(PRIMARY_ID) ON DELETE CASCADE
> ) ENGINE=InnoDB ROW_FORMAT=DYNAMIC;
> ```
> JDBC Session 이 기본적으로 사용하는 테이블 명은 `SPRING_SESSION` 이기 때문에 기본 설정된 테이블명으로 만들어도 된다.
